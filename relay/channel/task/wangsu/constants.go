...
// matchPriceTable 查找匹配的模型价格表，优先精确匹配，再使用最长前缀匹配（Longest Prefix Match）
func matchPriceTable(modelName string) (map[videoPriceKey]float64, bool) {
	name := strings.ToLower(strings.TrimSpace(modelName))
	if prices, ok := videoPriceTable[name]; ok {
		return prices, true
	}
	var bestKey string
	var bestPrices map[videoPriceKey]float64
	for k, prices := range videoPriceTable {
...
func GetVideoInputRatio(modelName, resolution string, hasVideo bool) (float64, bool) {
	prices, ok := matchPriceTable(modelName)
	if !ok {
		return 0, false
	}
	...
	price, ok := prices[videoPriceKey{
		is1080p: res == "1080p" || strings.Contains(res, "1080"),
		is4k:     res == "4k" || strings.Contains(res, "2160") || strings.Contains(res, "4k"),
		hasVideo: hasVideo,
	}]
	if !ok {
		// 未配置的组合按基准价计费
